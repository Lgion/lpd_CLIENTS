

const date = <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-calendar-date" viewBox="0 0 16 16">
    <path d="M6.445 11.688V6.354h-.633A13 13 0 0 0 4.5 7.16v.695c.375-.257.969-.62 1.258-.777h.012v4.61zm1.188-1.305c.047.64.594 1.406 1.703 1.406 1.258 0 2-1.066 2-2.871 0-1.934-.781-2.668-1.953-2.668-.926 0-1.797.672-1.797 1.809 0 1.16.824 1.77 1.676 1.77.746 0 1.23-.376 1.383-.79h.027c-.004 1.316-.461 2.164-1.305 2.164-.664 0-1.008-.45-1.05-.82zm2.953-2.317c0 .696-.559 1.18-1.184 1.18-.601 0-1.144-.383-1.144-1.2 0-.823.582-1.21 1.168-1.21.633 0 1.16.398 1.16 1.23"/>
    <path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5M1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4z"/>
</svg>
/*, event = <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 129.14 122.88" style="enable-background:new 0 0 129.14 122.88" xml:space="preserve">
<style type="text/css">
    {"<![CDATA[.st0{fill-rule:evenodd;clip-rule:evenodd;}]]>"}
</style>

<g>
    <path class="st0" d="M4.74,0H126.7c1.33,0,2.44,1.09,2.44,2.44v8.26c0,1.33-1.09,2.44-2.44,2.44h-1.77v53.53 c0,0.97-0.4,1.86-1.04,2.5l0,0c-0.64,0.64-1.54,1.04-2.51,1.04h-25.1v4.72h-5.17v7.33c-1.52-2.54-3.42-4.51-5.5-5.88 c-2.48-1.63-5.22-2.44-7.99-2.43c-2.79,0.01-5.52,0.86-7.99,2.52c-2.66,1.8-4.98,4.58-6.58,8.31h-9.08 c-0.17-0.06-0.35-0.13-0.53-0.19c-1.53-0.54-3.13-0.8-4.67-0.8c-1.53,0-3.12,0.26-4.67,0.8c-0.18,0.06-0.35,0.13-0.53,0.19h-3.31 v-9.86h-5.17v-4.72H10.06c-0.98,0-1.87-0.4-2.51-1.04c-0.06-0.06-0.12-0.13-0.17-0.19c-0.55-0.62-0.87-1.44-0.87-2.31V13.14H4.74 c-1.33,0-2.44-1.09-2.44-2.44V2.44C2.3,1.09,3.39,0,4.74,0L4.74,0L4.74,0z M69.51,107.71c-10.57,0-3.62,0.83-2.17-10.67 c2.15-17.19,18.5-17.19,20.91,0c1.54,11.06,8.18,10.67-2.17,10.67h-4.13c-0.02,2.82-0.45,4.26,2.47,5.83 c1.49,0.81,4.27,1.41,6.63,2.34c3.61-3.09,10.04-1.05,10.7-6.72c0.11-0.86-1.93-4.15-2.4-5.73c-1-1.6-1.36-4.12-0.26-5.8 c0.43-0.67,0.25-3.11,0.25-4.03c0-9.15,16.05-9.16,16.05,0c0,1.16-0.26,3.29,0.36,4.19c1.05,1.52,0.51,4.22-0.38,5.63 c-0.57,1.66-2.73,4.79-2.54,5.73c1.55,7.92,12.36-0.56,12.36,13.7c-41.72,0-83.45,0-125.18,0c0-14.49,13.68-6.47,14.73-15.35 c0.12-0.97-2.17-4.66-2.69-6.42c-1.11-1.79-1.52-4.62-0.29-6.5c0.48-0.75,0.28-3.48,0.28-4.52c0-10.27,17.98-10.27,17.98,0 c0,1.3-0.29,3.69,0.41,4.71c1.18,1.7,0.57,4.73-0.42,6.32c-0.64,1.86-3.06,5.37-2.86,6.42c0.76,3.84,10.32,6.44,13.65,7.85 c1.88-0.97,3.39-2.11,3.55-3.44c0.08-0.69-1.54-3.31-1.91-4.57c-0.8-1.27-1.08-3.29-0.21-4.63c0.35-0.54,0.2-2.48,0.2-3.22 c0-7.3,12.81-7.31,12.81,0c0,0.92-0.21,2.63,0.28,3.35c0.84,1.22,0.41,3.37-0.3,4.5c-0.45,1.32-2.18,3.83-2.03,4.57 c0.95,4.79,6.13,2.07,8.58,5.42c2.31-1.82,6.91-2.32,9.72-4c2.56-1.53,2.13-3.08,2.11-5.63L69.51,107.71L69.51,107.71L69.51,107.71 L69.51,107.71z M32.2,53.01c38.53,0,28.21-0.02,67.04-0.02c0.9,0,1.64,0.74,1.64,1.64v10.02c0,0.49-0.22,0.95-0.58,1.25h20.34 V13.41H10.8v52.51h20.34c-0.36-0.3-0.58-0.75-0.58-1.25V54.65C30.56,53.75,31.3,53.01,32.2,53.01L32.2,53.01L32.2,53.01z"/>
</g>
</svg>
*/
, meal = <svg class="icon x128" width="128" height="128" role="img" aria-labelledby="cis-restaurant-x4">
    <title id="cis-restaurant-x4">Restaurant Solid Icon</title>
    {/* <use xlink:href="/sprites/solid.svg#cis-restaurant"></use> */}
</svg>
, participants = <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-people-fill" viewBox="0 0 16 16">
<path d="M7 14s-1 0-1-1 1-4 5-4 5 3 5 4-1 1-1 1zm4-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6m-5.784 6A2.24 2.24 0 0 1 5 13c0-1.355.68-2.75 1.936-3.72A6.3 6.3 0 0 0 5 9c-4 0-5 3-5 4s1 1 1 1zM4.5 8a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5"/>
</svg>
, infos = <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-vcard-fill" viewBox="0 0 16 16">
<path d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2zm9 1.5a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 0-1h-4a.5.5 0 0 0-.5.5M9 8a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 0-1h-4A.5.5 0 0 0 9 8m1 2.5a.5.5 0 0 0 .5.5h3a.5.5 0 0 0 0-1h-3a.5.5 0 0 0-.5.5m-1 2C9 10.567 7.21 9 5 9c-2.086 0-3.8 1.398-3.984 3.181A1 1 0 0 0 2 13h6.96q.04-.245.04-.5M7 6a2 2 0 1 0-4 0 2 2 0 0 0 4 0"/>
</svg> 


const ok = <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-calendar-date" viewBox="0 0 16 16">
    <path d="M6.445 11.688V6.354h-.633A13 13 0 0 0 4.5 7.16v.695c.375-.257.969-.62 1.258-.777h.012v4.61zm1.188-1.305c.047.64.594 1.406 1.703 1.406 1.258 0 2-1.066 2-2.871 0-1.934-.781-2.668-1.953-2.668-.926 0-1.797.672-1.797 1.809 0 1.16.824 1.77 1.676 1.77.746 0 1.23-.376 1.383-.79h.027c-.004 1.316-.461 2.164-1.305 2.164-.664 0-1.008-.45-1.05-.82zm2.953-2.317c0 .696-.559 1.18-1.184 1.18-.601 0-1.144-.383-1.144-1.2 0-.823.582-1.21 1.168-1.21.633 0 1.16.398 1.16 1.23"/>
    <path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5M1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4z"/>
</svg>

export default function MobileChoices() {
    console.log(Date);
    return <ul className="dates" onClick={e => {
        // console.log(item.textContent==e.target.textContent)
        // console.log(item.textContent)
        // console.log(e.target.textContent)
        if (e.target.nodeName == "LI" || e.target.nodeName == "A") {
            let lis = Array.from(e.target.closest('ul').querySelectorAll('li'))
                , f = Array.from(bolobiForm.querySelectorAll('fieldset'))
                , li = lis.find((item, i) => item.textContent == e.target.textContent)
                , fieldsetClassName = li.className
            // console.log(li)
            lis.forEach((item, i) => item.classList.remove('active'))
            li.classList.add('active')
            f.forEach((item, i) => item.classList.remove('active'))
            document.querySelector("fieldset." + fieldsetClassName).className = "active " + document.querySelector("fieldset." + fieldsetClassName).className
        }
    }}>
        <h2>FORMULAIRE RÉSERVATION: </h2>
        <p style={{fontSize:"15px",color:"red"}}>Demander à l'IA de générer des svg pour chaque li de ce ul</p>
        <li className="active dates"><a href="#dates">{date} Dates</a></li>
        <li className="type"><a href="#type">{/*event*/} Évènement</a></li>
        <li className="location"><a href="#location">{participants} Nombre</a></li>
        <li className="meal"><a href="#meal">{meal} Repas</a></li>
        <li className="infos"><a href="#infos">{infos} Infos</a></li>
    </ul>
}
